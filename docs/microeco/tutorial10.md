...
| Prisoner B | Silent | Talk    |
| ---------- | -------- | --------- |
|**Prisoner A**|||
| Silent | A=1, B=1 | A=5, B= 0 |
| Talk | A=0, B=5 | A=3, B=3  |
...
1. 市场上仅有两家厂商（厂商 1 和厂商 2），生产完全同质的产品，无产品差异；
2. 两家厂商同时决定产量，不存在先后顺序（静态博弈）；
3. 厂商知道市场需求曲线（假设为线性需求：$P=a−bQ$，其中$Q=Q1+Q2$，*Q*1为厂商 1 产量,*Q*2为厂商 2 产量）；
4. 厂商的边际成本恒定（设为$MC1=MC2=c$），无固定成本；
5. 每个厂商都认为对手的产量固定，通过调整自身产量实现利润最大化（“古诺猜想”）。

在这里，你可以视为厂商A先制定了价格与产量，厂商B因此相应制定自己的价格与产量，在这里厂商A就是价格*领导者*，厂商B就是价格*追随者*。在这些情况下，策略的相互影响将构成序贯博弈。[^2]
